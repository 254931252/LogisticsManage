...
/**
 * @author Junhua
 * @ClassName:TasksService.java
 * @Version 版本
 * @ModifyedBy 修改人
 * @Copyright 公司名称
 * @date 2016年12月13日下午2:45:29
 */
public interface TasksService {
	/**
	 * 查询添加任务单（新订单）--（调度中心）
	 */
	List<OrderToTask> newOrderList(int top,int down,String createDate,String subName);
	/**
	 * 查询添加任务单（新订单）记录数--（调度中心）
	 */
	Integer newOrderCount(String createDate,String subName);
	/**
	 * 查询订单详情--（调度中心）
	 */
	
	OrderToTask oneOrder(Integer ordid);
	/**
	 * 添加任务单--(调度中心）
	 */
	 ...
	 * 需修改任务单状态的订单--（调度中心）
	 */
	List<OrderToTask> distpachUpdate(@Param("top") Integer top, @Param("down") Integer down);
	/**
	 * 需修改任务单状态的订单记录数
	 */
	...
	List<ShowTask> allTask(@Param("top") Integer top, @Param("down") Integer down,@Param("taskId") Integer taskId,
			@Param("createDate")	String createDate,@Param("sender") Integer sender,@Param("orderState") Integer orderState,
	 ...
	 * 删除一条任务单
	 */
	void doDelete(@Param("id") Integer id);
	/**
	 * 分配或中转任务
	 */
